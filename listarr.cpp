...
{
	dataItems = new DataType[maxSize]; // 전체 리스트 메모리 할당
	// pre-lab
...
{
	if (isFull()) cout << "Full List " << endl; // 리스트가 꽉찬 경우

	else if (isEmpty()) {//리스트가 빈 경우
		...
		for (int i = 0; i < size; i++){
			for (int i = 0; i < size; i++) {
				if (dataItems[i] > dataItems[i + 1]){
					...
					int a = 1;
					int b = 2;
					memory = new DataType[maxSize]; //리스트 메모리 할당
...
{
	if(isEmpty()) cout << "Empty List " << endl; // 리스트가 빈 경우
	else {
